(** Groupe Fikret KURKLU, Tom KACHA, Hugo MEIGNEN **)
(** j'ai fais reduction_Pcarre_2 ainsi que SN_SN' et SN'_SN**)
...
Proof.
  ...
  { cbv [corps_boucle] .
    ...
    apply SN_Assign .}
  cbn .
    {rewrite plus_n_Sm . rewrite <- minus_n_O . apply Hrec_x .}
Qed.
...
Proof.
  ...
  - cbn . eapply SN_While_true .
    ...
    cbn .
    Compute (evalB (Bnot (Beqnat (Ava 0) (Aco 2)))[2;4;5]) . 
...
Proof.
  ...
  induction sn as  [ (* SN_Skip *) s
                   ...
                   | (* SN_While_true *) cond i s0 s1 s' e sn hrec_sn sni hrec_sni
 ...
 (** La preuve qui suit est un peu fastidieuse, on verra plus tard
  ...
  - destruct cond .
    -- apply hrec_sn .
    -- cbn in e .
       pose ( f(b:bool) := if (b) then i1 else i2 ) .
       change ( SN(simpl_test_Btrue_Bfalse (f(false))) s s').
       rewrite e .
       change (SN (simpl_test_Btrue_Bfalse i1) s s') .
       apply hrec_sn .
    -- apply SN_If_true .
...
(** Dans l'exercice qui suit, on aura un but comprenant
    ...
    prédicat ad-hoc suivant, qui isole le cas intéressant de SN.
 *)
...
Proof.
  ...
  induction sn as  [ (* SN_Skip *) s
                   ...
                   | (* SN_If_false *) cond i1 i2 s s' e sn hrec_sn
                   | (* SN_While_false *) cond i s e
                   | (* SN_While_true *) cond i s0 s2 e sn hrec_sn1
                   ].
  - apply SN'_Skip .
  - apply SN'_Assign .
  -  eapply SN'_Seq .
     -- apply hrec_sn1 . 
     -- apply hrec_sn2 .    
  - apply SN'_If_true . rewrite e . reflexivity . apply hrec_sn.
  - apply SN'_If_false . rewrite e . reflexivity . apply hrec_sn .
  - apply SN'_While_false . rewrite e . reflexivity .
  - eapply SN'_While_true .
    -- rewrite sn . reflexivity .
    -- eapply SN'_Seq .
       --- apply IHhrec_sn1 .
       --- apply IHsn1 .
Qed.
...
Proof.
  ...
  induction sn' as [ (* SN_Skip *) s
                   ...
                   | (* SN_If_false *) cond i1 i2 s s' e sn hrec_sn
                   | (* SN_While_false *) cond i s e
                   | (* SN_While_true *) cond i s0 s2 e sn hrec_sn
                   ].
  - apply SN_Skip .
  - apply SN_Assign .
  -  eapply SN_Seq .
     -- apply hrec_sn1 . 
     -- apply hrec_sn2 .    
  - apply SN_If_true . rewrite e . reflexivity . apply hrec_sn .
  - apply SN_If_false . rewrite e . reflexivity . apply hrec_sn .
  - apply SN_While_false . rewrite e . reflexivity .
...
| SNr_Repeat_true : forall b i1 i2 s s2,
                    evalB b s = true -> SNr i2 s s2 -> SNr (Repeat i1 b) s s2
| SNr_Repeat_false: forall b i1 i2 s s2,
                    evalB b s = false -> SNr i2 s s2 -> SNr (Repeat i1 b) s s2
.
...
Repeat SNr_Seq (Amo (Ava i) 1).(* a faiiiiire*)
...
(** Remarque : on pourrait également considérer un langage WHILE_REPEAT
    comprenant à l fois l'nstruction While et l'instruction Repeat
 *)
...
Proof.
  ...
  { cbn . eapply SOS_again .
    ...
    - cbn  . eapply SOS_again .
      -- apply SOS_Assign .
      -- cbn . rewrite <- Sn_2 . rewrite <- Sn_carre .
         apply SOS_stop . }
Qed.
...
Proof.
  ...
  { apply SOS_While .}
  { eapply SOS_again .
    - apply SOS_If_true . cbn . rewrite S . cbn . reflexivity .
  ...
  - eapply SOS_again .
    ...
    -- eapply SOS_again .
       ...
       --- cbn . eapply SOS_again .
           ...
           ---- cbn . eapply SOS_again .
                -----  apply SOS_Seqf . apply SOS_Assign .
                ----- cbn . rewrite <- Sn_2 . rewrite <- Sn_carre . apply SOS_stop . 
Qed.
...
Fixpoint f_SOS_1 (i : winstr) (s : state) : config :=
  ...
  | Seq i1 i2 => let s1:= f_SOS_1 i1 s in
                 match s1 with
                 | Final x => Inter i2 x
                 | Inter i1' x => Inter (Seq i1' i2) x
                 end
  | If e i1 i2 => match evalB e s with
                  | true  => Inter i1 s
                  | false => Inter i2 s
                  end
  | (While e i1) => (Inter (If e (Seq i1 (While e i1)) Skip) s)
  end.
...
Definition PC4 := Seq incrY Pcarre_2 .
...
Proof.
  ...
  Compute (f_SOS_1 PC1 [0;0;1]) .
  apply SOS_again with (f_SOS_1 PC1 [0;0;1]) . cbn .
  { apply SOS_If_true . cbn . reflexivity . }
  apply SOS_again with (f_SOS_1 PC2 [0;0;1]) . cbn .
  { apply SOS_Seqi . cbn . apply SOS_Seqf .  apply SOS_Assign . }
  Compute (f_SOS_1 PC2 [0; 0; 1]) .
  apply SOS_again with (f_SOS_1 PC3 [1;0;1]) . cbn .
  { apply SOS_Seqi . apply SOS_Seqf .  apply SOS_Assign . }
  Compute (f_SOS_1 PC3 [1; 0; 1]) .
  apply SOS_again with (f_SOS_1 PC4 [1;1;1]) . cbn .
  { apply SOS_Seqf . apply SOS_Assign . }
  cbn . apply SOS_stop .
Qed .
...
Proof.
  ...
  { apply SOS_Skip .}
  { apply SOS_Assign . }
  { induction (f_SOS_1 (Seq i1 i2) s) as [] .
    -
Admitted.
